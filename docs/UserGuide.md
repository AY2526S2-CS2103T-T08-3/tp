...
A: Copy the `data/addressbook.json` file from your current Client2Door folder to the `data/` folder on the other computer. The [`export`](#exporting-driver-delivery-assignments--export) command creates an HTML delivery summary, not a full backup, and cannot be loaded using [`import`](#importing-subscribers--import).
...
| **Export** | `export [FILE_PATH]`                                                                  | `export march-delivery.html`                   |
